...
const sql_getById = `
    SELECT cli_id, cli_cnpj, cli_nome, cli_ddi, cli_ddd, cli_fone
      FROM t_cliente
     WHERE cli_id = $1`;
...
const getClienteById = async (id) => {
    try {
        ...
        return { total: result.rows.length, cliente: result.rows[0] };
    } catch (err) {
...
const getClientes = async () => {
    try {
        ...
        return { total: result.rows.length, clientes: result.rows };
    } catch (err) {
...
const sql_post = `
    ...
    VALUES ($1, $2, $3, $4, $5) RETURNING cli_id`;
...
const postCliente = async (params) => {
    try {
        ...
        return { mensagem: 'Cliente criado com sucesso!', id: result.rows[0].cli_id };
    } catch (err) {
        throw { status: 500, message: 'Erro ao tentar criar Cliente' };
    }
};
...
const sql_put = `
    UPDATE t_cliente
       SET cli_cnpj = $2,
           ...
           cli_ddd  = $5,
           cli_fone = $6
     WHERE cli_id = $1 RETURNING cli_id`;
...
const sql_patch = `UPDATE t_cliente SET`;
...
const patchCliente = async (params) => {
    try {
        ...
        if (params.cnpj) {
            count++;
            fields += ` cli_cnpj = $${count}`;
            binds.push(params.cnpj);
        }
        if (params.nome) {
            count++;
            fields += (fields ? ',' : '') + ` cli_nome = $${count}`;
            binds.push(params.nome);
        }
        if (params.ddi) {
            count++;
            fields += (fields ? ',' : '') + ` cli_ddi = $${count}`;
            binds.push(params.ddi);
        }
        if (params.ddd) {
            count++;
            fields += (fields ? ',' : '') + ` cli_ddd = $${count}`;
            binds.push(params.ddd);
        }
        if (params.fone) {
            count++;
            fields += (fields ? ',' : '') + ` cli_fone = $${count}`;
            binds.push(params.fone);
        }

        let sql = sql_patch + fields + ' WHERE cli_id = $1 RETURNING cli_id';
